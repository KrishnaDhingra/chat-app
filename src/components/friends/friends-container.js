...
function FriendsContainer({ onSelect }) {
  ...
  const filterFriends = async (userName) => {
    // let filteredFriends = totalFriends.filter((friend) => {
    //   let displayName2 = await getUsername(friend.friendId)
    //   displayName2.includes(userName.toLowerCase())
    // })
    // filteredFriends
  ...
  const filterAddFriends = (userName) => {
    ...
    let filteredAddFriends = totalAddFriends.filter((friend) =>
      friend.displayName.toLowerCase().includes(userName.toLowerCase()),
    )

    filteredAddFriends
      ? setFilteredAddFriends(filteredAddFriends)
      : setFilteredAddFriends([])
  }
  return (
    <main className="z-0 bg-chatContainer flex flex-col gap-6 h-[100vh] lg:max-h-[92vh] lg:w-full py-7 pl-4 sm:pl-10 rounded-r-md m-0">
      <section className="flex flex-col gap-6 w-full pr-4 sm:pr-10">
        <section className="flex w-full">
          <p
            onClick={() => toggleFriendsScreen(true)}
        ...
        {friendsScreen ? (
          <SearchBar onChange={filterFriends} />
        ) : (
          <AddFriendSearchBar onChange={filterAddFriends} />
        )}
      </section>
